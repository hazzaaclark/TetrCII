...
int main(int argc, char* argv[])
{
	...
	while (!GAME::GAME_END)
	{
		...
		if (GAME::FORCE_DOWN)
		{
			...
			/* IF THERE ARE NO LINES, CREATE NEW PIECES */

			while (GAME::VECTOR_LINES.empty())
			{
				GAME::SCORE += (1 << GAME::VECTOR_LINES.size()) * 100;
			}

			GAME::CURRENT_X_POS = GAME::FIELD_WIDTH / 2;
			GAME::CURRENT_Y_POS == 0;
			GAME::CURRENT_ROTATION == 0;
			GAME::CURRENT_PIECE = rand() % 7;
			GAME::GAME_END = !PIECE_FIT(GAME::CURRENT_PIECE, GAME::CURRENT_ROTATION, GAME::CURRENT_X_POS, GAME::CURRENT_Y_POS);
			...
			swprintf_s(&SCREEN_BUFF[2 * SCREEN::WIDTH + GAME::FIELD_WIDTH + 6], 16, L"SCORE: %%", GAME::SCORE);
		}
	}
}
